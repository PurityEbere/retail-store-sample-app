...
def replace_images_with_ecr(obj, opts):
    ...
    # Handle Service resources (moved out of the if block)
    elif kind == "Service":
        ...
        if "ui" in name:
            ...
            obj["spec"] = spec 
            pulumi.log.info(f"Converted service '{name}' to LoadBalancer on port 80")
...
for service in services:
    ...
    if Path(kompose_files_dir).exists():
        ...
        if all_files:
            ...
            pulumi.log.warn("Skipping {service}: Directory exists but no YAML files found")
    else:
        pulumi.log.warn("Skipping {service}: Directory not found at {kompose_files_dir}")
...
if ui_service:
    ui_svc = k8s.core.v1.Service.get(
        "ui-lb-service",
        pulumi.Output.concat(namespace_name, "/ui"),
        opts=pulumi.ResourceOptions(provider=k8s_provider)
    )
    pulumi.export("ui_url", ui_svc.status.apply(
        lambda s: f"http://{s.load_balancer.ingress[0].hostname if s and s.load_balancer and s.load_balancer.ingress else 'pending'}"
    ))